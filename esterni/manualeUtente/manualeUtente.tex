% importa variabili globali
\input{../../template/config.tex}

% definizione variabili locali
\def\DOCUMENTO{Manuale Utente}
\def\VERSIONE{2.0.0}

\def\DESCRIZIONE{Documento che facilita l'utilizzo dell'applicazione da parte dell'utente.}

\def\REDATTORE {Agostinetto Matteo}
\def\VERIFICATORE {Crespan Emanuele}
\def\RESPONSABILE {Suierica Bogdan}

\def\USO {Esterno}

\def\DISTRIBUZIONE {\GRUPPO{}\\ & \COMMITTENTE{}\\ & \PROPONENTE{}\\}


% abilita (true) / disabilita (false) indice, lista tabelle, lista figure
\def\INDICE	{true}
\def\TABELLE {true}
\def\FIGURE {true}


% importa struttura
\input {../../template/structure.tex}

\input{diario_modifiche.tex}

% importa indici
\input {../../template/index.tex}

\section{Introduzione}
\input{sections/introduzione.tex}
\newpage

\section{Premi}
\input{sections/start.tex}
\newpage

\section{Ricerca di un progetto}
\input{sections/ricerca.tex}
\newpage

\section{Creazione di un progetto}
\input{sections/creazione_progetto.tex}

\section{Eliminazione di un progetto}
\input{sections/eliminazione_progetto.tex}

\section{Rinominazione di un progetto}
\input{sections/rinominazione_progetto.tex}

\section{Stampa ed esportazione di una presentazione}
\input{sections/stampa_presentazione.tex}


\section{Creazione di una presentazione}
\input{sections/creazione_presentazione.tex}
\newpage


\section{Editor presentazioni}
\input{sections/editor_presentazioni.tex}
\newpage


\section{Visualizzazione presentazioni}
\input{sections/visualizzazione_presentazioni.tex}
\newpage

\section{Segnalazione degli errori}
\input{sections/segnalazioni.tex}
\newpage


\section{Glossario}
\input{sections/glossario_man.tex}
\newpage

%\newpage

% ...

%\printglossaries

\end{document}
