...
\begin{center}
\begin{tabular}{|l|p{0.3\textwidth}|p{0.3\textwidth}|c|}
		TU11 & Viene verificato che venga rimossa una slide e vengano riposizionale le altre nella posizione corretta & removeSlide() & N.A.\\
	\midrule
		TU12 & Viene verificato che venga caricata correttamente la slide & loadSlide(direction: String) & Success\\
	\midrule
		TU13 & Viene verificato che venga salvata correttamente la slide corrente nel database & saveSlide() & Success\\
	\midrule
		TU14 & Viene verificato che vengano restituiti tutti i progetti appartenenti allo user \textit{username} & searchUsers(username:String) & Success\\
	\midrule
		TU15 & Viene verificato che vengano restituiti tutti i progetti con nome \textit{name} & searchProject(name:String) & Success\\
		\midrule
		TU16 & Viene verificato che vengano retiruiti tutti i progetti di che hanno nome \textit{name} o appartinenti all'utente \textit{username} & search(query:String) & Success\\
	\midrule
		TU17 & Verifica che il metodo di registrazione salvi correttamente i dati sul database & register(username:String,password:String) & Success\\
	\midrule
		TU18 & Viene verificato che venga effettuata correttamente la registrazione tramite account di socialNetwork Facebook e GooglePlus & registerViaFacebook(),registerViaGooglePlus() & N.A.\\
	\midrule
		TU19 & Viene verificato che il componente venga aggiornato correttamente il componente indetificato da \textit{id} nell'area di disegno della slide corrente & udpateObject(id:String) & Success\\
	\midrule
		TU20 & Viene verificato che l'utente venga registrato correttamente nel server & signUp(email: String, username: String, password: String, passwordCheck:String) & Success\\
\end{tabular}
\end{center}
\end{table}
\begin{table}[h]
